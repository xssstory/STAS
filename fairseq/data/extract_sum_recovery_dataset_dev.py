...
# sent sep included!
# [cls] w1 w2 ... [sep]
def get_docs(samples, vocab, maxlen=None, bert_model=None):
...
class ExtractSumRecoveryDevDataset(FairseqDataset):
    ...
    def collate(self, samples, src_dict, tgt_dict, left_pad_source=True,
        ...
        if len(samples) == 0:
            return {}    
        docs, cls_pos = get_docs(samples, src_dict, self.max_sent_len, bert_model=self.bert_model)
        id = torch.LongTensor([s['id'] for s in samples])

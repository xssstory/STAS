...
def create_src_tok_batch(samples, src_dict, max_doc_length=None, max_sent_length=None, rng=None):
...
class SentsPermutationWithMaskDataset(FairseqDataset):
    ...
    def collate(self, samples, src_dict, tgt_dict, left_pad_source=True, left_pad_target=False, is_poniter_net=False, max_doc_len=None):
        ...
        src_tokens, doc_pad_mask, segment_ids, cpos, nsents, tgt_orders = create_src_tok_batch(samples, src_dict, max_doc_length=max_doc_len, max_sent_length=self.max_sent_len+1, rng=self.rng)

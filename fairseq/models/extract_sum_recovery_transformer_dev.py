...
@register_model('extract_sum_recovery_dev')
class ExtractSumRecoveryTransformer(FairseqModel):
    ...
    def forward(self, bsz, max_nsents, masked_docs_generator, doc_without_mask):
        ...
        scores = [torch.zeros([bsz, max_nsents], dtype=torch.float32).cuda() for _ in range(len(para_list) * len(self.args.temperature))]
        if self.args.fp16:
        ...
        with torch.no_grad():
            ...
            for idx, masked_docs_input in enumerate(masked_docs_generator):
                ...
                masked_docs_input['net_input']['doc_pad_mask'].fill_(True)
                masked_docs_input['net_input']["doc_pad_mask"][:, idx] = False
                encoder_out_2, decoder_out_2 = self.single_forward(**masked_docs_input["net_input"])

                lprobs = self.get_normalized_probs(decoder_out, log_probs=False, idx=0) 
                # lprobs = self.get_normalized_probs(decoder_out, log_probs=True, idx=0)   
                lprobs = lprobs.view(-1, lprobs.size(-1))

                lprobs_old = self.get_normalized_probs(decoder_out, log_probs=False, idx=0).view(-1, lprobs.size(-1))
                lprobs_2 = self.get_normalized_probs(decoder_out_2, log_probs=False, idx=0).view(-1, lprobs.size(-1))

                if self.args.fp16:
                    lprobs_old = lprobs_old.half()
                    lprobs = lprobs.half()
                    lprobs_2 = lprobs_2.half()
                ...
                recovery_score_old = lprobs_old.gather(dim=-1, index=target).view(non_pad_mask.shape).masked_fill_(non_pad_mask==0, 0).sum(dim=-1) / non_pad_mask.sum(dim=-1).type_as(lprobs)    
                # recovery_score = torch.exp(lprobs.gather(dim=-1, index=target).view(non_pad_mask.shape).masked_fill_(non_pad_mask==0, 0).sum(dim=-1) / non_pad_mask.sum(dim=-1).type_as(lprobs)) 
                ...
                self_score = lprobs_2.gather(dim=-1, index=target).view(non_pad_mask.shape).masked_fill_(non_pad_mask==0, 0).sum(dim=-1) / non_pad_mask.sum(dim=-1).type_as(lprobs)
                self_score.masked_fill_(nsents<=idx, 0) 
                # new_recovery_score = F.relu(recovery_score - self_score) / self_score
                # new_recovery_score = F.relu(recovery_score - self_score)
                # recovery_score = new_recovery_score.masked_fill(recovery_score==0, 0)
                ...
                attn_weights_before_softmax = encoder_out['attn_weights']
                ...
                for i, temperature in enumerate(self.args.temperature):
                    ...
                    for j, (lam1, lam2) in enumerate(para_list):
                        attn_score = get_attn_score(attn_weights_avg_all_layer, idx)
                        # attn_score = get_attn_score(attn_weights_final_layer_avg, idx)

                        # num_sents = (doc_pad_mask==0).sum(dim=1, keepdim=True) - 1
                        # num_sents = num_sents.half() if self.args.fp16 else num_sents.float()
                        # thresh = attn_score.sum(dim=1, keepdim=True) / num_sents
                        # attn_score = F.relu(attn_score - thresh)
                        # attn_score = attn_score / attn_score.sum(dim=1, keepdim=True)

                        scores[i * len(para_list) + j][:, idx] += recovery_score.view(-1) * lam1
                        scores[i * len(para_list) + j] += attn_score * recovery_score * lam2
                        # scores[i * len(para_list) + j] += attn_score * lam2
...
@register_model_architecture('extract_sum_recovery_dev', 'extract_sum_recovery_dev')
...
@register_model_architecture('extract_sum_recovery_dev', 'extract_sum_recovery_dev_medium')
...
@register_model_architecture('extract_sum_recovery_dev', 'extract_sum_recovery_dev_base')
def transformer_vaswani_wmt_en_de_big(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 768)
    args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 3072)
    args.encoder_attention_heads = getattr(args, 'encoder_attention_heads', 12)
...
# Medium size transformer
@register_model_architecture('extract_sum_recovery_dev', 'extract_sum_recovery_dev_large')

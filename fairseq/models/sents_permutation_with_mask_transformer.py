...
@register_model('sents_permutation_with_mask')
class SentsPermutationWithMaskTransformer(FairseqEncoderDecoderModel):
    ...
    def forward(self, src_tokens, src_tokens_with_mask, segment_ids, doc_pad_mask, doc_pos_tok, cls_pos, cls_pos_mask, masked_sent_positions, prev_output_tokens, prev_output_tokens_perm):
        encoder_out_perm = self.encoder(src_tokens, segment_ids, doc_pad_mask, doc_pos_tok, cls_pos)
        decoder_out_perm = self.decoder_perm(prev_output_tokens_perm, encoder_out_perm)
        encoder_out_mask = self.encoder(src_tokens_with_mask, segment_ids, doc_pad_mask, doc_pos_tok, cls_pos_mask)
    ...
    def __init__(self, args, dictionary, embed_tokens, left_pad=False):
        ...
        if self.predict_arch == 'pointer_net':
            if self.pointer_net_attn_type == 'perceptron':
                ...
                nn.init.normal_(self.pointer_attn_weight, mean=0, std=embed_dim)
...
#     def __init__(self, args, src_dictionary, embed_tokens, left_pad=False):
#         '''
...
@register_model_architecture('sents_permutation_with_mask', 'sents_permutation_with_mask')
...
@register_model_architecture('sents_permutation_with_mask', 'sents_permutation_with_mask_medium')
...
@register_model_architecture('sents_permutation_with_mask', 'sents_permutation_with_mask_large')
def transformer_medium(args):

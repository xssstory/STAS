...
ctx.reducer.dump = pickle4reducer.dump
...
def main(args, init_distributed=False):
    ...
    for valid_sub_split in args.valid_subset.split(','):
        task.load_dataset(valid_sub_split, shuffle=False, combine=False, epoch=0)
...
def train(args, trainer, task, epoch_itr):
    ...
    progress = progress_bar.build_progress_bar(
        args, itr, epoch_itr.epoch, default='simple', no_progress_bar='simple',
    )
...
def validate(args, trainer, task, epoch_itr, subsets):
    ...
    for subset in subsets:
        ...
        for idx, sample in enumerate(progress):
            ...
            if hasattr(args, 'validate_step') and args.validate_step is not None:
                if idx > args.validate_step:
                    break
    ...
    for k, v in new_args.__dict__.items():
        if k not in no_update_args and not k.startswith('distributed'):
            if getattr(args, k, None) != v:
                print('| WARNING: update {} in args from {} to {}'.format(k, getattr(args, k, None), v))
                setattr(args, k, v)
...
def cli_main():
    ...
    # if 'pac_nyt' in args.data:
    #     args.max_epoch *= 5
    # if args.task == 'sum2article':
    #     update_args(args)
    if 'EnglishGigaword' in args.data:
        args.save_interval_updates = 1000
        print('training on the gigaword, set the save_interval_updates to 1000')
